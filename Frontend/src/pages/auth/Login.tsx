        ...
        try {
            ...
            fetch('http://127.0.0.1:7242/ingest/3226bb8d-dcf5-4e14-b71b-e9e5cab1d50f', {
                ...
                body: JSON.stringify({
                    runId: 'pre-fix',
                    hypothesisId: 'H6',
                    location: 'src/pages/auth/Login.tsx:handleLoginStart',
            ...
            if (step === 'credentials') {
                const res = await api.post('/auth/login-initial', { email, password });
                setTwofaToken(res.data.twofaToken);
                setStep('2fa');
            } else {
                if (!twofaToken) {
                    setError('Missing 2FA session. Please try logging in again.');
                    setStep('credentials');
                    return;
                }
                const res = await api.post('/auth/login-verify-2fa', { code, twofaToken });
            ...
            fetch('http://127.0.0.1:7242/ingest/3226bb8d-dcf5-4e14-b71b-e9e5cab1d50f', {
                ...
                body: JSON.stringify({
                    runId: 'pre-fix',
                    hypothesisId: 'H6',
                    location: 'src/pages/auth/Login.tsx:handleLoginError',
                    message: 'Login error in frontend',
                    data: {
                ...
                <form onSubmit={handleLogin}>
                    ...
                    <CardFooter className="flex flex-col space-y-4">
                        <Button type="submit" className="w-full" isLoading={loading}>
                            {step === 'credentials' ? 'Continue' : 'Verify 2FA & Sign In'}
                        </Button>
                        {step === '2fa' && (
                            <button
                                type="button"
